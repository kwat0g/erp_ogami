...
INSERT INTO users (id, email, username, password, first_name, last_name, role, is_active) VALUES
('admin-001', 'admin@erp.local', 'admin', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'System', 'Administrator', 'ADMIN', TRUE);
...
('Purchase Order Approval', 'Purchasing', 'PURCHASE_ORDER', 2, 'Two-level approval for purchase orders'),
...
-- Insert approval workflow steps for Purchase Requisition
INSERT INTO approval_workflow_steps (workflow_id, step_number, step_name, approver_role) 
SELECT id, 1, 'Department Head Approval', 'DEPARTMENT_HEAD' FROM approval_workflows WHERE document_type = 'PURCHASE_REQUISITION';

INSERT INTO approval_workflow_steps (workflow_id, step_number, step_name, approver_role) 
SELECT id, 2, 'Purchasing Manager Approval', 'PURCHASING' FROM approval_workflows WHERE document_type = 'PURCHASE_REQUISITION';

-- Insert approval workflow steps for Purchase Order
INSERT INTO approval_workflow_steps (workflow_id, step_number, step_name, approver_role) 
SELECT id, 1, 'Purchasing Manager Approval', 'PURCHASING' FROM approval_workflows WHERE document_type = 'PURCHASE_ORDER';

INSERT INTO approval_workflow_steps (workflow_id, step_number, step_name, approver_role) 
SELECT id, 2, 'Top Management Approval', 'TOP_MANAGEMENT' FROM approval_workflows WHERE document_type = 'PURCHASE_ORDER';
