...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!canApprove) {
    return res.status(403).json({ message: 'You do not have permission to reject requisitions' });
  }
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (pr.status !== 'PENDING' && pr.status !== 'DRAFT') {
        return res.status(400).json({ message: 'Only pending requisitions can be rejected' });
      }
      ...
      return res.status(200).json({ message: 'Purchase requisition rejected' });
    } catch (error) {
